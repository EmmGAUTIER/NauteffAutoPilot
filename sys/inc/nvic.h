...
_inline void NVIC_enable_IRQ (unsigned irqn)
{
    NVIC_ISER[irqn / 32] |= 0x1 << (irqn%32U);
}

_inline void NVIC_disable_IRQ (unsigned irqn)
{
    NVIC_ISER[irqn / 32] &= ~(0x1 << (irqn%32U));
}
#if 0
/*Sets the pending status of interrupt or exception to 1. */
_inline void NVIC_set_pending_IRQ (unsigned IRQn)
{
    ;
}

/*  Clears the pending status of interrupt or exception to 0.*/
_inline void NVIC_clear_pending_IRQ (unsigned IRQn)
{
    ;
}
 ...
 */
_inline bool NVIC_get_pending_IRQ_status (unsigned IRQn)
{
    ;
}
#endif
 ...
 */
_inline void NVIC_SetPriority (unsigned IRQn, uint32_t priority) {
