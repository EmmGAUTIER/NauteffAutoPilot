...
inline void GPIO_pin_set (GPIO_TypeDef *port, unsigned pin)
{
    port->BSRR = (0x1 << pin);
}

__attribute__((always_inline))
inline void GPIO_pin_reset (GPIO_TypeDef *port, unsigned pin)
{
 ...
 */
__attribute__((always_inline))
inline void GPIO_pins_set (GPIO_TypeDef *port, uint32_t mask)
{
    port->BSRR = (mask);
}

__attribute__((always_inline))
inline void GPIO_pins_reset (GPIO_TypeDef *port, uint32_t mask)
{
    port->BSRR = (mask << 16U);
}

__attribute__((always_inline))
inline int GPIO_pin_get (GPIO_TypeDef *port, int pin)
{
    return (port->IDR & (0x1 << pin)) ? 1 : 0;
}

__attribute__((always_inline))
inline uint32_t GPIO_pins_get (GPIO_TypeDef *port, uint32_t mask)
{
