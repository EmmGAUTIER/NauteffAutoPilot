...
static float zeta = 0.001f;

float SEq_1 = 0.866025, SEq_2 = 0.f, SEq_3 = 0.f, SEq_4 = 0.5f;
float b_x = 1, b_z = 0;
...
void madgwick_get_Euler_angles(float *proll, float *ppitch, float *pyaw)
{
    *proll = roll;
    *ppitch = pitch;
    *pyaw = yaw;
...
/**
 * Filtre de Madgwick - Mise à jour avec gyroscope, accéléromètre et magnétomètre
 *
 * @param gx, gy, gz : Vitesses angulaires du gyroscope (rad/s)
 * @param ax, ay, az : Accélérations (g ou m/s², l'unité importe peu tant qu'elle est cohérente)
 * @param mx, my, mz : Champ magnétique (gauss ou µT, l'unité importe peu)
 * @param dt : Période d'échantillonnage (secondes)
 */

void madgwick_update(float w_x, float w_y, float w_z,
