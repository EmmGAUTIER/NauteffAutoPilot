...
static inline Vector3f Vector3f_Scale(Vector3f v, float s)
{
    Vector3f r = {v.x * s, v.y * s, v.z * s};
    return r;
}
static inline float Vector3f_Dot(Vector3f a, Vector3f b)
{
    return a.x * b.x + a.y * b.y + a.z * b.z;
}
static inline float Vector3f_Norm(Vector3f v) { return sqrtf(Vector3f_Dot(v, v)); }
...
static inline Vector3f Matrix3f_MultVec(const Matrix3f *A, const Vector3f *v)
{
    Vector3f r;
...
static inline Matrix3f Matrix3f_Mult(const Matrix3f *A, const Matrix3f *B)
{
    Matrix3f R;
    for (int i = 0; i < 3; i++)
        for (int j = 0; j < 3; j++)
...
static inline Matrix3f Matrix3f_Transpose(const Matrix3f *A)
{
    Matrix3f T;
    for (int i = 0; i < 3; i++)
        for (int j = 0; j < 3; j++)
...
int Matrix3f_Inverse(const Matrix3f *A, Matrix3f *inv)
{
